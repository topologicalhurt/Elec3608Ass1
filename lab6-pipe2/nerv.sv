...
module nerv #(
	...
	// --------------------------
	// 2-stage pipeline additions
	// --------------------------
	// Stage-2 instruction register and branch-delay machinery
	logic [31:0] ir;
	wire  [31:0] pc_ex = imem_addr_q;  // PC of instruction in stage 2
	logic        br_taken_q, br_taken_d;
	logic [31:0] br_target_q, br_target_d;

	// instruction memory pointer
	assign imem_addr = (trap || mem_rd_enable_q) ? imem_addr_q : npc;
	// stage-2 instruction comes from IR (latched each cycle)
	assign insn = ir;
	...
	wire [20:1] imm_j_enc = {insn[31], insn[19:12], insn[20], insn[30:21]}; // as in original
	wire [20:0] imm_j;
	assign {imm_j[20], imm_j[10:1], imm_j[11], imm_j[19:12], imm_j[0]} = {insn[31:12], 1'b 0};
	...
	always_comb begin
		...
		// default branch-latch inputs (computed in stage 2 this cycle,
		// applied to PC selection next cycle to enforce one delay slot)
		br_taken_d  = 1'b0;
		br_target_d = 'hx;
		...
		case (insn_opcode)
			...
			OPCODE_BRANCH: begin
				// Compute branch decision/target in stage 2 (using pc_ex),
				// but DO NOT change npc here. The redirect is applied next
				// cycle (via br_taken_q) so the delay-slot always executes.
				case (insn_funct3)
					3'b 000 /* BEQ  */: br_taken_d = (rs1_value == rs2_value);
					3'b 001 /* BNE  */: br_taken_d = (rs1_value != rs2_value);
					3'b 100 /* BLT  */: br_taken_d = ($signed(rs1_value) <  $signed(rs2_value));
					3'b 101 /* BGE  */: br_taken_d = ($signed(rs1_value) >= $signed(rs2_value));
					3'b 110 /* BLTU */: br_taken_d = (rs1_value <  rs2_value);
					3'b 111 /* BGEU */: br_taken_d = (rs1_value >= rs2_value);
					default: illinsn = 1;
				endcase
				br_target_d = pc_ex + imm_b_sext;
			end
		...
		// apply last cycle's branch decision to next fetch address
		// (this enforces exactly one delay slot)
		if (br_taken_q) begin
			npc = br_target_q;
			if (npc & 32'b 11) begin
				illinsn = 1;
	...
	always @(posedge clock) begin
		...
		// latch IF->EX instruction and the branch result for next-cycle PC select
		if (!reset) begin
			if (!mem_rd_enable_q) begin
				ir <= imem_data;
			end
